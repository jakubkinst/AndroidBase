...
public class ImageUtils {
	...
	public static Bitmap decodeSampledBitmapFromFile(File file, int reqWidth,
		...
		final int ss = options.inSampleSize;

		// Decode bitmap with inSampleSize set
		options.inJustDecodeBounds = false;
		final Bitmap b = BitmapFactory.decodeFile(file.getAbsolutePath(),
				options);
		// JKUtils.logError("Requested: "+reqWidth+"x"+reqHeight+". SampleSize: "+ss+". Bitmap size: "+b.getWidth()+"x"+b.getHeight());
		return b;
	}
	...
	public static void makeImageViewLive(ImageView image) {
		float pivot = (float) (Math.random() * Math.min(image.getHeight(),
				image.getWidth()));
		ScaleAnimation animIn = new ScaleAnimation(1f, 1.2f, 1f, 1.2f, pivot,
				pivot);
		animIn.setDuration(15000);
		animIn.setRepeatMode(Animation.REVERSE);
